...
import ApplicationActions from '../../../../Lib/v0.6/src/Base/enums/Actionms/ApplicationActions';
import { IAppConfig } from '../components/interfaces/IAppConfig';
import { setLanguage } from "../../../../Lib/v0.6/src/Base/components/core_nls";
...
export class InitConfig<T>{
    ...
    public resolveConfig(configPath: string): Promise<IAppConfig> {
        let minVersion = 0.4;
        return new Promise((resolve, reject) => {
            axios.get(configPath)
                .then((response) => {
                    ...
                    // eval(response.data);
                    if (config.sitemerties.version < minVersion) {
                    ...
                    appConfig.configUrl = configPath;
    ...
    public configureStore(rootReducer: Reducer, production: boolean): Store {
        let baseMiddleware: Array<Middleware> = [thunk];
        window.log = production ? () => { } : console.log;
        ...
        const middleware = production ? baseMiddleware : baseMiddleware.concat([reduxLogger]);
